...
public class ArrayConverter extends Converter<Object> {
	...
	public static int getIntBytesLength() {
		if (IntBytesLength == -1)
			IntBytesLength = ConversionTools.convert(Integer.MAX_VALUE).length;
	...
	public Object fromBytes(byte[] rep, Class<?> type) {
		...
		int length = ConversionTools.convert(int.class, tmpBytes), objLength;
		...
		for (int i = 0; i < length; ++i) {
			...
			objLength = ConversionTools.convert(int.class, tmpBytes);
