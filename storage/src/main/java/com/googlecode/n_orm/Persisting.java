 ...
 * </p><p>Other (non static, non transient, non final) attributes are to be stored in the data store (with no additional annotations required).
 ...
 * Non-key properties and column families are also stored in tables for superclasses unless {@link #storeAlsoInSuperClasses()} is set to true.
...
public @interface Persisting {
	...
	/**
	 * States whether values (properties and column families) should also be stored in superclasses.
